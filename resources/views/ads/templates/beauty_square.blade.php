...
<div style="width: 500px; height: 500px; background-color: {{ $data['bg_color'] ?? '#f9f5f0' }}; font-family: 'Poppins', sans-serif; position: relative; overflow: hidden; border: 1px solid #eee;">
    ...
    <div style="position: absolute; top: 55px; left: 45px; z-index: 50; width: 280px;">
        ...
        {{-- Services List - Shifted slightly up --}}
        <div style="margin-top: 25px;">
            ...
            <ul style="list-style: none; padding: 0; margin: 0; font-size: 15px; font-weight: 600; color: {{ $data['service_color'] ?? '#8d5524' }}; line-height: 1.5;">
                <li style="display: flex; align-items: center; gap: 8px;">• {{ $data['service_1'] ?? 'Skin Care' }}</li>
                <li style="display: flex; align-items: center; gap: 8px;">• {{ $data['service_2'] ?? 'Facial Treatment' }}</li>
                <li style="display: flex; align-items: center; gap: 8px;">• {{ $data['service_3'] ?? 'Body Treatment' }}</li>
            </ul>
        </div>
    </div>
    ...
    {{-- Floating Contact Card - Lowered to prevent overlap --}}
    {{-- Floating Contact Card --}}
    <div style="position: absolute; bottom: 20px; left: 25px; width: 180px; background: #ffffff00; padding: 15px; border-radius: 6px; z-index: 100;">
        ...
        <div style="font-size: 13px; font-weight: 700; color: #fffaf8; line-height: 1.3;">
            ...
            {{-- Updated this line to use 'address' instead of 'ig_handle' --}}
