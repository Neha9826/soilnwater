    ...
    <div class="max-w-[1100px] mx-auto px-4">
        <div style="display: flex; gap: 30px; align-items: flex-start;">
            ...
            <div style="flex: 0 0 65%; max-width: 65%;" class="space-y-6">
                ...
                <div x-data="{ active: 0 }">
                    <div class="bg-white rounded-[2.5rem] overflow-hidden shadow-sm border border-gray-200 aspect-square">
                        @if(count($images) > 0)
                ...
                <div class="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                        {{-- Safely handle amenities whether array or object --}}
                        @php $amenities = is_array($property->amenities) ? $property->amenities : []; @endphp
                        @forelse($amenities as $amenity)
                            <div class="flex items-center gap-2">
                                <i class="fas fa-check text-green-500 text-[9px]"></i>
                                <span class="text-[10px] font-bold text-gray-700 uppercase">{{ is_string($amenity) ? $amenity : ($amenity['name'] ?? 'Amenity') }}</span>
                            </div>
                        @empty
                            <p class="text-[10px] text-gray-400 italic font-bold">Standard Amenities Included</p>
                        @endforelse
                    </div>
                </div>
            </div>
            ...
            <div style="flex: 0 0 32%; max-width: 32%; position: sticky; top: 20px;" class="space-y-4">
                <div class="bg-white rounded-[2.5rem] p-6 border border-gray-100 shadow-xl">
                    <div class="mb-6">
                        <span class="bg-blue-600 text-white text-[8px] font-black px-2 py-0.5 rounded uppercase inline-block mb-3">Verified Property</span>
                        ...
                        <p class="text-[9px] font-bold text-gray-400 uppercase flex items-center gap-1">
                            <i class="fas fa-map-marker-alt text-green-500"></i> {{ $property->location ?? $property->city }}
                        </p>
                    </div>

                    <div class="bg-gray-50 rounded-2xl p-4 mb-6 border border-gray-100">
                        <p class="text-[8px] font-black text-gray-400 uppercase mb-0.5">Price</p>
                    ...
                    <div class="grid grid-cols-2 gap-4 pt-6 border-t border-gray-100 text-[9px] font-bold uppercase">
                        ...
                        <div>
                            <span class="text-gray-300 text-[8px] block mb-1">Status</span>
