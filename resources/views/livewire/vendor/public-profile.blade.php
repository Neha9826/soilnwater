<div class="min-h-screen bg-white font-sans text-gray-800">
    ...
    <div class="relative w-full mb-24">
        ...
        <div class="relative h-[60vh] w-full bg-gray-900 overflow-hidden" 
             x-data="{ 
                activeSlide: 0, 
            ...
            <div class="absolute bottom-0 left-0 w-full p-6 md:p-12 z-10 flex flex-col md:flex-row items-end justify-between gap-6">
                ...
                <div class="md:pl-52 w-full md:w-2/3 text-white mb-4 md:mb-0">
                    ...
                    <div class="flex items-center gap-3 mt-4">
                        <div class="text-xl md:text-2xl font-bold tracking-wide text-white border-l-4 border-blue-500 pl-3">
                            {{ $vendor->store_name }}
                        </div>
                        @if($vendor->is_vendor)
                            <i class="fas fa-check-circle text-blue-400 text-xl md:text-2xl drop-shadow-md" title="Verified Vendor"></i>
                        @endif
                    </div>
                </div>
                ...
                <div class="flex gap-3 mb-6 md:mb-2">
                    ...
                    @if($vendor->phone)
                    <a href="https://wa.me/91{{ $vendor->phone }}" target="_blank" class="bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-full font-bold shadow-lg transition flex items-center gap-2">
        ...
        <div class="absolute bottom-[-5rem] left-6 md:left-12 z-30">
            <div class="h-32 w-32 md:h-44 md:w-44 bg-white rounded-2xl shadow-2xl p-2 border-4 border-white hover-shake transition-transform duration-300">
    ...
    <div class="sticky top-28 z-40 bg-white border-b border-gray-100 shadow-sm pl-6 md:pl-64 pr-6 transition-all duration-300">
                ...
                <div class="flex flex-col md:flex-row gap-12 items-center {{ $index % 2 == 1 ? 'md:flex-row-reverse' : '' }}">
                    ...
                    <div class="flex-1 space-y-6 w-full">
                        <div class="flex items-center gap-4 mb-2">
                            ...
                            <span class="text-sm font-bold text-blue-600 uppercase tracking-widest">Our Story</span>
                        </div>
    ...
    <div id="products" class="bg-gray-50 py-20 mt-12 border-t border-gray-200">
        <div class="max-w-7xl mx-auto px-6">
            <h2 class="text-4xl font-bold text-gray-900 mb-12 text-center">Our Products</h2>
                    ...
                    <div class="bg-white rounded-2xl p-4 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 group cursor-pointer border border-gray-100">
                        <div class="h-56 bg-gray-100 rounded-xl overflow-hidden mb-4 relative">
                             @if(!empty($product->images))
                                <img src="{{ asset('storage/'.$product->images[0]) }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500">
                            @endif
    ...
    <div id="contact" class="bg-gray-900 text-white py-16">
        <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-12">
            <div>
                ...
                @if($vendor->phone) <p class="flex items-center gap-3"><i class="fas fa-phone text-green-400"></i> +91 {{ $vendor->phone }}</p> @endif
            </div>
            <div class="flex md:justify-end items-center gap-4">
                @if($vendor->facebook) <a href="{{ $vendor->facebook }}" target="_blank" class="text-white hover:text-blue-500 text-3xl transition"><i class="fab fa-facebook"></i></a> @endif
                @if($vendor->instagram) <a href="{{ $vendor->instagram }}" target="_blank" class="text-white hover:text-pink-500 text-3xl transition"><i class="fab fa-instagram"></i></a> @endif
            </div>
        </div>
    </div>
</div>