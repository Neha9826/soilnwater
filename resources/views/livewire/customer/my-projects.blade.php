    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 border-b border-gray-200 pb-6">
            ...
            <div class="w-full md:w-auto">
                <h1 class="text-3xl font-extrabold text-gray-900">My Projects</h1>
                <p class="text-gray-500 text-sm mt-1">Manage your upcoming and active projects.</p>
            </div>
            ...
            <div class="w-full md:w-auto flex flex-col sm:flex-row gap-3">
                ...
                <div class="flex gap-2 flex-shrink-0">
                    ...
                    {{-- Post Button --}}
                    <a href="{{ route('customer.project.create') }}" 
        ...
        <div class="min-h-[400px]">
                            ...
                            <div class="bg-white rounded-2xl shadow-sm hover:shadow-lg transition-shadow border border-gray-200 overflow-hidden flex flex-col h-full group">
                                <a href="{{ route('customer.project.edit', $project->id) }}" class="block flex-1">
                                    <div class="h-64 w-full bg-gray-200 relative overflow-hidden">
                                        ...
                                        @endif
                                        <div class="absolute top-3 left-3"><span class="bg-blue-600 text-white text-[10px] font-bold px-2 py-1 rounded shadow-sm uppercase">{{ $project->project_status }}</span></div>
                                        <div class="absolute bottom-3 right-3"><span class="bg-gray-900/80 text-white text-sm font-bold px-3 py-1.5 rounded-lg shadow-sm">₹{{ number_format($project->price) }}</span></div>
                                    </div>
                                    <div class="p-5">
                                        <div class="flex gap-2 mb-3">
                                            <span class="px-2 py-1 bg-blue-50 text-blue-700 text-[10px] font-bold uppercase rounded border border-blue-100">{{ $project->type }}</span>
                                        </div>
                                        <h3 class="font-bold text-gray-900 text-lg mb-2 line-clamp-1 group-hover:text-blue-600 transition">{{ $project->title }}</h3>
                                        <p class="text-sm text-gray-500 flex items-center gap-1.5"><i class="fas fa-map-marker-alt text-gray-400"></i> {{ $project->city }}</p>
                                    </div>
                                </a>
                                <div class="px-5 py-4 bg-gray-50 border-t flex justify-between items-center text-xs text-gray-500">
                                    ...
                                    <button wire:click="delete({{ $project->id }})" onclick="confirm('Delete this project?') || event.stopImmediatePropagation()" class="text-gray-400 hover:text-red-600 transition p-2 hover:bg-red-50 rounded-full"><i class="fas fa-trash-alt text-sm"></i></button>
                                </div>
                            </div>
                        @endforeach
                    </div>
                                ...
                                <div class="p-6 flex-grow flex flex-col justify-between">
                                    <div>
                                        <div class="flex justify-between items-start mb-2">
                                            <h3 class="text-xl font-bold text-gray-900 group-hover:text-blue-600 transition"><a href="{{ route('customer.project.edit', $project->id) }}">{{ $project->title }}</a></h3>
                                            <span class="text-xl font-extrabold text-gray-900">₹{{ number_format($project->price) }}</span>
                                        </div>
                                        <p class="text-sm text-gray-500 flex items-center gap-1 mb-3"><i class="fas fa-map-marker-alt text-gray-400"></i> {{ $project->city }}</p>
                                        <span class="px-2.5 py-1 bg-blue-50 text-blue-700 rounded text-xs font-bold border border-blue-100">{{ $project->project_status }}</span>
                                    </div>
                                    <div class="flex justify-between items-center mt-4 pt-4 border-t border-gray-100">
                                        <span class="text-xs text-gray-500">Added {{ $project->created_at->format('d M, Y') }}</span>
                                        <div class="flex gap-3">
                                            <a href="{{ route('customer.project.edit', $project->id) }}" class="text-blue-600 hover:text-blue-700 text-sm font-bold flex items-center gap-1"><i class="fas fa-edit"></i> Edit</a>
                                            <button wire:click="delete({{ $project->id }})" onclick="confirm('Delete?') || event.stopImmediatePropagation()" class="text-red-500 hover:text-red-700 text-sm font-bold flex items-center gap-1"><i class="fas fa-trash-alt"></i> Delete</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            @else
                ...
                <div class="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border-2 border-dashed border-gray-300">
                    <div class="bg-gray-50 rounded-full h-24 w-24 flex items-center justify-center mb-4"><i class="fas fa-project-diagram text-4xl text-gray-400"></i></div>
                    <h3 class="text-xl font-bold text-gray-900">No Projects Found</h3>
                    <p class="text-gray-500 mt-2 mb-6 max-w-sm text-center">You haven't posted any upcoming projects or societies yet.</p>
                    <a href="{{ route('customer.project.create') }}" class="bg-blue-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-blue-700 transition shadow-lg flex items-center gap-2">
