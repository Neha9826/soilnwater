...
class CategorySeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($vendors as $name) {
            Category::firstOrCreate(['name' => $name, 'type' => 'vendor'], ['is_active' => true]);
        }
        ...
        foreach ($consultants as $name) {
            Category::firstOrCreate(['name' => $name, 'type' => 'consultant'], ['is_active' => true]);
        }

        // 3. Hotel/Resort Categories
        $hospitality = ['Hotel', 'Resort', 'Homestay', 'Guest House'];
        foreach ($hospitality as $name) {
            Category::firstOrCreate(['name' => $name, 'type' => 'hotel'], ['is_active' => true]);
        }
        ...
        foreach ($services as $name) {
            Category::firstOrCreate(['name' => $name, 'type' => 'service'], ['is_active' => true]);
        }
        
        // 5. Builder Categories
        $builders = ['Residential Developer', 'Commercial Builder', 'Civil Contractor'];
        foreach ($builders as $name) {
            Category::firstOrCreate(['name' => $name, 'type' => 'builder'], ['is_active' => true]);
        }
    }
}